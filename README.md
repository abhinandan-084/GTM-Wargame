...
- **[LLM fabrication rates](benchmark/results/experiment_a_report.md)** (Experiment A, real API calls, run locally, never in CI): how often real models write a number that isn't in the ground truth, with the guardrail observing. Across 30 paired seeds (identical scenarios per model, analyst + strategist nodes scored): **Gemini flagged in 6/30 runs** (95% CI [9.5%, 37.3%]; 13 of 537 numbers written, 2.4%) and **local llama.cpp (Llama 3.1 8B Q4) flagged in 4/30 runs** (95% CI [5.3%, 29.7%]; 10 of 138 numbers, 7.2%). Every flag came from the strategist node, and every flagged transcript is committed under [`benchmark/results/examples/`](benchmark/results/examples/). This is not a leaderboard: the frontier model's non-zero rate shows fabrication persists even at the frontier under prompts that forbid inventing numbers, and the local model's higher per-number rate shows why the guardrail is load-bearing in local/private deployments.
